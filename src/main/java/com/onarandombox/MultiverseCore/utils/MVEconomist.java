...
public class MVEconomist {
     ...
     *
     * @param currency the type of currency. A value other than -1 indicates the material type used for currency.
     * @return true if currency is valid material
     */
    public static boolean isItemCurrency(String currency) {
    	return Material.getMaterial(currency) != null;
    }
    ...
    private static class ItemEconomy {
        ...
        private static boolean hasEnough(Player player, double amount, String currency) {
            if (isItemCurrency(currency)) {
            	Material m = Material.getMaterial(currency);
                return player.getInventory().contains(m, (int) amount);
            } else {
                return true;
            }
        }
        ...
        private static void deposit(Player player, double amount, String currency) {
            if (isItemCurrency(currency)) {
                giveItem(player, amount, Material.getMaterial(currency));
            }
        }

        private static void withdraw(Player player, double amount, String currency) {
            if (isItemCurrency(currency)) {
                takeItem(player, amount, Material.getMaterial(currency));
            }
        }
        ...
        private static void giveItem(Player player, double amount, Material type) {
            ...
            showReceipt(player, (amount * -1), type.toString());
        }
        ...
        private static void takeItem(Player player, double amount, Material type) {
            ...
            showReceipt(player, amount, type.toString());
        }
